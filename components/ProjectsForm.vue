...
import {getClient, literal, Pact, readKeyset} from "@kadena/client";
import {nanoid} from "nanoid";
import {PactNumber} from "@kadena/pactjs";
import {useKdaUsd} from "~/composables/useKdaUsd";
...
const submitForm = async () => {
    ...
    const unisignedTransaction = Pact.builder.execution(
    ...
    const signedTransaction = await signTransaction(unisignedTransaction);


    const kadenaClient = getClient(
        ({chainId , networkId})=> `http://127.0.0.1:8080/chainweb/0.0/${networkId}/chain/${chainId}/pact`);
   ...
   const transactionResult = pollResult[requestKey].result;
     ...
     if (transactionResult.status === 'failure') {
      ...
      const { uuid } = await createProjectInDB({
        ...
        image: form.image || 'https://placehold.co/500/320',
