...
<template>
  <div class="fixed bottom-0 z-10 w-full p-2 right-2 sm:p-0 sm:w-auto sm:bottom-5" >
    <TransitionGroup name="list" tag="div">
      ...
      <div v-for="alert in alerts" :key="alert.id" class="mb-4 border-t-[1px] shadow-sm alert !pr-0" :class="`border-t-${alert.type}`" >
        <div class="flex justify-between sm:w-80">
          ...
          <button v-if="alert.dismissiable"  @click="dismiss(alert)" class="btn btn-ghost p-1 transition rounded hover:bg-white" ></button>
        </div>
      </div>
    </TransitionGroup>
  </div>
